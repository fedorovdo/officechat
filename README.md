...
Sidebar activity indicators combine local previews/recent ordering with authoritative server-side unread counters for groups, direct chats, and discussions. A local sidebar search filters groups by name or slug and users by display name or username. The normal direct-message list hides inactive users, the current user, and bot users.
