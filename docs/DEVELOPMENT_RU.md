...
В top bar user app доступна отдельная панель профиля. Она показывает сведения об аккаунте и позволяет активному авторизованному пользователю изменить собственное отображаемое имя через `PATCH /api/auth/me`. Загрузка аватара и сохранение UI-настроек в backend пока не реализованы.
...
В composer `Enter` отправляет сообщение, `Shift+Enter` вставляет новую строку, а `Ctrl+Enter` сохраняется для обратной совместимости. Выбор группового вложения открывается компактной кнопкой рядом с полем ввода.
