...
export function AdminBackups({ dictionary, locale }: AdminBackupsProps) {
  ...
  return (
    <AdminPageShell ariaLabel={text.title} className="admin-backups-page" wide>
      <AdminPageHeader
        actions={<button className="admin-button admin-button-secondary" disabled={refreshing} onClick={() => void loadAll(page)} type="button">{refreshing ? text.refreshing : text.refresh}</button>}
