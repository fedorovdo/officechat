...
Browser Notifications v0.1 добавляет базовые уведомления браузера для новых сообщений, когда вкладка OfficeChat не активна. Это frontend-only механизм: пользователь включает уведомления в настройках app shell, браузер должен выдать разрешение, а вкладка OfficeChat должна оставаться открытой. Service worker, server push и уведомления без открытой вкладки запланированы позже.
