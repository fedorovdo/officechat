...
Soft delete сообщения немедленно закрывает доступ ко всем его вложениям в PostgreSQL. Публичные API возвращают `attachments=[]`, а прежний download URL отвечает `404` для автора, участников и администраторов. После успешного commit backend удаляет отдельный файл из local storage; ошибка файловой системы не возвращает доступ к вложению, а оставшийся orphan можно безопасно убрать повторной cleanup-командой.

Проверка существующих вложений удалённых сообщений выполняется без изменений:

```bash
docker compose exec backend python -m app.cli cleanup-deleted-attachments
```

В отчёте указываются число attachment-записей, найденных файлов и суммарный размер. Для фактического закрытия legacy-записей и удаления только файлов удалённых сообщений нужен явный флаг:

```bash
docker compose exec backend python -m app.cli cleanup-deleted-attachments --apply
```

Активные сообщения и их вложения команда не затрагивает.
