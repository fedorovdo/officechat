...
Unread/read-state API можно проверить после входа через `GET /api/unread` и `POST /api/read-state`. Подтверждаемая пользовательская операция `POST /api/read-state/mark-all-current-read` исправляет только исторически зависшие unread counters текущего пользователя; она не нужна для обычного чтения сообщений, не затрагивает будущие сообщения и сохраняет немесседжевые уведомления. Миграция `20260704_0017` не переписывает сообщения: она создаёт только high-water rows и считает существующую историю прочитанной. Для проверки скрытой вкладки и cross-tab synchronization используйте два профиля браузера; read actions намеренно не появляются в Audit Log.
...
Системные уведомления включаются только явным действием в настройках app shell. Настройка дублируется в `localStorage` (`officechat.notifications.enabled`) и backend preferences. Уведомления работают для открытого, в том числе свёрнутого или неактивного окна после разрешения браузера и ОС. В Chromium PWA `navigator.setAppBadge()` показывает authoritative unread total. Подробная инструкция: `docs/NOTIFICATIONS_RU.md`.

Текущая версия получает события через `WS /api/ws/me`, включая group/direct/discussion messages. Service Worker отсутствует, поэтому полностью закрытое приложение уведомления не получает; Web Push/VAPID, email и mobile push запланированы отдельно.
...
wizard импорта XLSX/CSV. После preview выполняется детерминированная сверка,
ручной выбор create/update/skip и dry validation. `DirectoryEntry`
изменяется только после отдельного подтверждения, одной транзакцией; пустые
значения не перезаписывают существующие поля, а stale matches блокируют execute.
Настройки ограничений находятся в `.env.example` с prefix
`DIRECTORY_IMPORT_`.
...
docker compose exec backend python -m pytest -q tests/test_directory_import_reconciliation.py
