...
Файл также можно перетащить в активный group/direct/discussion panel. Drop overlay не закрывает sidebar, а drop только выбирает первый файл. Текст, reply и composer остаются без изменений; новый файл заменяет предыдущее единственное вложение.
