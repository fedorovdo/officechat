...
Текущие групповые уведомления зависят от WebSocket подключений к доступным группам в открытом app shell. Для надежной доставки персональных уведомлений по всем типам событий позже планируется `WS /api/ws/me`. Service worker, server push и уведомления без открытой вкладки запланированы позже.
