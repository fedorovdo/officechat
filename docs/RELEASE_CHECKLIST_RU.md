...
- `OFFICECHAT_RELEASE_VERSION` задан явно и совпадает с annotated tag.
- Backend `/health`, frontend `/api/health` and About show exact release version.
- `OFFICECHAT_VERSION=<VERSION> docker compose -f deploy/docker-compose.release.yml config`.
- `bash -n scripts/release/*.sh` and `bash -n scripts/release/officechatctl`.
- `OFFICECHAT_RELEASE_VERSION=<VERSION> bash scripts/release/create-release-bundle.sh --dry-run`.
- Release archive name is `officechat-<VERSION>-linux-amd64.tar.gz`.
