...
Pinned Messages v0.1 работает для group, direct и discussion сообщений. REST API сообщений возвращает поля `is_pinned`, `pin_id` и `pinned_at`, а отдельный endpoint `GET /api/pins?chat_type=...&chat_id=...` возвращает список закреплений с коротким preview сообщения.

Закреплять, откреплять и менять заметку может только активный не-bot пользователь с эффективным правом `can_pin_messages` и обычным доступом к чату. Удалённые и архивные сообщения не закрепляются; при удалении или архивации сообщения закрепление снимается автоматически. Лимит задаётся `PINNED_MESSAGES_MAX_PER_CHAT`.

Frontend `/ru/app` показывает compact pinned strip над списком сообщений, список всех закреплений, короткие заметки и переход к исходному сообщению через existing jump-to-message. Подробнее: [PINNED_MESSAGES_RU.md](PINNED_MESSAGES_RU.md).
...
Сообщение может содержать до `ATTACHMENT_MAX_FILES_PER_MESSAGE=10` локальных вложений. Суммарный лимит задаётся `ATTACHMENT_MAX_TOTAL_SIZE_MB=50`, per-file лимит — `ATTACHMENT_MAX_UPLOAD_SIZE_MB=25`. Файлы сохраняются в Docker volume `UPLOADS_DIR=/data/uploads` под безопасными уникальными именами; оригинальное имя остаётся metadata.

PNG/JPEG/WebP screenshot через `Ctrl+V` добавляется к текущему выбору. File picker, clipboard и drag-and-drop используют общий список вложений с индивидуальным удалением и общей очисткой. Обычная вставка текста, `Enter`, `Shift+Enter`, reply и emoji picker работают как прежде.

Несколько файлов можно перетащить в активный group/direct/discussion panel. Drop overlay не закрывает sidebar, а drop добавляет все файлы в composer. Non-image файлы, включая `txt`, принимаются по extension allowlist даже при MIME `text/plain` или `application/octet-stream`.

После отправки PNG/JPEG/WebP образуют responsive gallery. Защищённый lightbox поддерживает навигацию по изображениям. Остальные форматы показываются компактными download rows; bytes в WebSocket не передаются.
...
- Нет antivirus scanning, resumable uploads и backend thumbnails/compression.
